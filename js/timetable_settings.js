  ...
  function show_timetable_options() {
    ...
    for (day_object of config.data.timetable) {
      timetable_settings_group.innerHTML +=
        "<div class='input_group'>" + `<input name='day_name_input' value='${day_object.day}'/>` + '</div>'
    ...
    function add_period_input_group(subject, room) {
      schedule_inputs_container.innerHTML +=
        ...
        `<input value='${subject}' placeholder="Предмет"/>` +
        `<input value='${room}' placeholder="Аудиторія"/>` +
        '</div>'
    }
  }
}
...
function save() {
  ...
  var schedule_input_group_containers = document.querySelectorAll(
